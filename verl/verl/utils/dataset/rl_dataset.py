...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        self.dataframe = self._filter_invalid_prompt_rows(self.dataframe)
        self.dataframe = self.maybe_filter_out_long_prompts(self.dataframe)

    def _filter_invalid_prompt_rows(self, dataframe: datasets.Dataset) -> datasets.Dataset:
        """Remove rows with malformed prompts to avoid dataloader runtime crashes."""

        prompt_key = self.prompt_key

        def _is_valid_prompt(doc) -> bool:
            prompt = doc.get(prompt_key, None)
            if prompt is None:
                return False
        ...
        dataframe = dataframe.filter(
            ...
            desc=f"Filtering invalid '{prompt_key}' rows",
        )
        after = len(dataframe)
        if after != before:
            print(f"filtered invalid prompt rows: {before - after} removed, {after} remaining")
        return dataframe
    ...
    def _build_messages(self, example: dict):
        ...
        if messages is None:
            raise ValueError(f"Missing prompt field '{self.prompt_key}' in dataset row.")
        ...
        if not isinstance(messages, list):
            raise TypeError(
                f"Prompt field '{self.prompt_key}' must be a list, but got type={type(messages).__name__}"
            )
    ...
    def __getitem__(self, item):
        ...
        # Convert tokenized fields to torch tensors if present.
        for key in ("input_ids", "attention_mask", "position_ids"):
            if key in row_dict and not isinstance(row_dict[key], torch.Tensor):
                row_dict[key] = torch.tensor(row_dict[key], dtype=torch.long)
