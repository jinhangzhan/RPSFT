...
import ast
from transformers import AutoProcessor, BitsAndBytesConfig, AutoModelForCausalLM, TrainerCallback
...
def configure_llm(model, training_args):
    # Handle both multimodal wrappers (with `language_model`) and plain LlamaForCausalLM
    if hasattr(model, "language_model"):
        ...
        llm = model
    set_requires_grad(llm.parameters(), not training_args.freeze_llm)
...
keep_kw = (".self_attn.q_proj.weight", ".self_attn.k_proj.weight", ".self_attn.v_proj.weight", ".self_attn.o_proj.weight", ".mlp.down_proj.weight", ".mlp.gate_proj.weight", ".mlp.up_proj.weight", ".lm_head.weight", ".embed_tokens.weight")
...
def train():
    ...
    baseline_params = {}  # optional: keep W0 full for debugging
    ...
    if training_args.svd_reg_coeff and training_args.svd_reg_coeff != 0.0:
        for name, p in tqdm(model.named_parameters(), desc="baseline"):
            ...
            baseline_params[key] = W.detach().cpu()  # optional

            min_dim = min(W.shape)
            k = min(topk, min_dim)
            if k == min_dim:
                # Full-rank case: use ||W - W0||^2 in the loss, skip SVD.
                continue
    ...
    if hasattr(model.config, "text_config"):
        # Hidden size for the text tower
        if hasattr(model.config.text_config, "hidden_size"):
            model.config.hidden_size = model.config.text_config.hidden_size
        # Disable cache during training
        model.config.text_config.use_cache = False
    else:
        # Plain LlamaForCausalLM: hidden_size already exists here
        # Just disable cache at top level
        model.config.use_cache = False

    print('training_args============')
    try:
        print(f"[SVD REG CONFIG] coeff={training_args.svd_reg_coeff} topk={training_args.svd_reg_topk} mode={getattr(training_args, 'svd_reg_mode', 'residual')}")
    except Exception:
        pass
    ...
    # Handle text-only case: processor might *be* the tokenizer
    if isinstance(processor, PreTrainedTokenizerBase):
        tokenizer = processor
        processor.tokenizer = tokenizer  # so the rest of the code can use processor.tokenizer
    else:
        tokenizer = processor.tokenizer

    # Make sure we have a pad token
    if tokenizer.pad_token_id is None:
        # Common choice for Llama-style models
    ...
    if sft_method == "kl":
        trainer_cls = LLamaKLLoraTrainer if training_args.lora_enable else LLamaKLSFTTrainer
        ...
        trainer_cls = LLamaDFTLoraTrainer if training_args.lora_enable else LLamaDFTTrainer
    elif sft_method == "iw":
        trainer_cls = LLamaIwSFTLoraTrainer if training_args.lora_enable else LLamaIwSFTTrainer
        if ref_model is None:
        ...
        trainer_cls = LLamaVLoraTrainer if training_args.lora_enable else LLamaVTrainer
    ...
    # eval_id = ConversationsEvalDataset("${SCRATCH}/data/SFTvsRL_Data/SFT_Data/gp-l/ind-eval_300.json", processor)
