...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        cot_value = getattr(self.data_args, "use_cot", None)
        use_cot_prompt = False
        if cot_value is not None:
            if isinstance(cot_value, bool):
                use_cot_prompt = cot_value
            elif isinstance(cot_value, str):
                normalized = cot_value.strip().lower()
                use_cot_prompt = normalized not in ("", "0", "false", "none")
            else:
                use_cot_prompt = bool(cot_value)

        system_message = None
        if use_cot_prompt:
            system_message = {
                "role": "system",
                "content": [{"type": "text", "text": COT_SYSTEM_PROMPT}],
            }
            sources_with_system = [system_message] + sources
        else:
            sources_with_system = sources
        ...
        # (This whole block is effectively ignored for pure text if `images` is None)
        if images is not None:
            input_text = processor.apply_chat_template(
                sources_with_system,
                tokenize=False,  # <<< make sure this returns a string
        ...
        for idx, j in enumerate(range(0, len(sources), 2)):
            ...
            if idx == 0:
                ...
                if system_message is not None:
                    first_turn_messages.append(system_message)
                first_turn_messages.append(user_input)

                # IMPORTANT: force text output
                user_prompt = processor.apply_chat_template(
                    first_turn_messages,
                    tokenize=False,  # <<< THIS IS THE KEY CHANGE
                    add_generation_prompt=True,
                )
            else:
                user_prompt = (
                    f"{START_HEADER_TOKEN}{user_input['role']}{END_HEADER_TOKEN}\n\n"
                    f"{user_input['content'][0]['text']}{EOT_TOKEN}\n"
                    f"{START_HEADER_TOKEN}{gpt_response['role']}{END_HEADER_TOKEN}\n\n"
                )

            # For a text model, user_prompt & gpt_prompt are now plain strings
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            token_len += example["input_ids"].shape[0]
...
def make_supervised_data_module(processor, data_args):
    ...
    eval_path = os.environ.get("SFT_EVAL_PATH")
    if not eval_path:
        return dict(train_dataset=sft_dataset,
                    data_collator=data_collator)
    eval_data_arg = DataArguments()
    eval_data_arg.data_path = eval_path
    eval_data_arg.max_num_frames = data_args.max_num_frames
    eval_data_arg.lazy_preprocess = data_args.lazy_preprocess
    eval_data_arg.use_cot = data_args.use_cot
    ood_dataset = LazySupervisedDataset(data_path=eval_path, processor=processor, data_args=eval_data_arg)
    return dict(train_dataset=sft_dataset,
